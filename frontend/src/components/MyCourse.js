...
function MyCourse() {
  ...
  const onReload = () => {
    const url = "http://localhost:8003/Enroll/userId/"+localStorage.getItem("id");
    axios
      .get(url)
      .then((response) => {
  ...
  const columns = [
    ...
    { 
      ...
      render: rowData => (
        <button onClick={() => handleViewCourse(rowData.course_details._id)} class="btn btn-primary">Click</button>
      )
    }
  ]
  return (
